...
type MediaMtxConfig struct {
	...
	// RunOnConnect is a command that runs when a client connects to the src.
	//
	// The following environment variables are available:
	//
	// * RTSP_PORT: RTSP src port
	//
	...
	// RunOnDisconnect runs when a client disconnects from the src.
	//
	// The following environment variables are available:
	//
	// * RTSP_PORT: RTSP src port
	...
	// Every time a  user wants to authenticate, the src will call the authentication method.
	//
	...
	// ApiServerKey sets the path to src key for the API.
	ApiServerKey string `yaml:"apiServerKey"`
	// ApiServerCert sets the path to src certificate for the API.
	...
	// ApiTrustedProxies is a list of IPs or CIDRs of proxies placed before the HTTP src
	ApiTrustedProxies []string `yaml:"apiTrustedProxies"`

	// Playback enable downloading recordings from the playback src.
	Playback bool `yaml:"playback"`
	// PlaybackAddress sets the port for the playback src (e.g. :9996).
	PlaybackAddress string `yaml:"playbackAddress"`
	// PlaybackEncryption enables the encryption for the playback src.
	PlaybackEncryption bool `yaml:"playbackEncryption"`
	// PlaybackServerKey sets the path to src key for the playback src.
	PlaybackServerKey string `yaml:"playbackServerKey"`
	// PlaybackServerCert sets the path to src certificate for the playback src.
	...
	// PlaybackTrustedProxies is a list of IPs or CIDRs of proxies placed before the playback src
	PlaybackTrustedProxies []string `yaml:"playbackTrustedProxies"`

	// Rtsp enables the RTSP src.
	Rtsp bool `yaml:"rtsp"`

	// Rtmp enables the RTMP src.
	Rtmp bool `yaml:"rtmp"`
	// RtmpAddress sets the port for the RTMP src (e.g. :1935).
	RtmpAddress string `yaml:"rtmpAddress"`
	// RtmpEncryption enables the encryption for the RTMP src.
	RtmpEncryption string `yaml:"rtmpEncryption"`
	// RtmpsAddress sets the port for the RTMPS src (e.g. :1936).
	RtmpsAddress string `yaml:"rtmpsAddress"`
	// RtmpServerKey sets the path to src key for the RTMP src.
	RtmpServerKey string `yaml:"rtmpServerKey"`
	// RtmpServerCert sets the path to src certificate for the RTMP src.
	RtmpServerCert string `yaml:"rtmpServerCert"`

	// Hls enables the HLS src.
	Hls bool `yaml:"hls"`

	// WebRTC enables the WebRTC src.
	WebRTC bool `yaml:"webRTC"`

	// Srt enables the SRT src.
	Srt bool `yaml:"srt"`
	// SrtAddress sets the port for the SRT src (e.g. :8890).
...
func (m *MediaMtxConfig) BuildEscortPath(flagship *Flagship, srtPort, rtspPort int) Paths {
	logger.HavenLogger.Debug().Msgf("Pulling SRT feed from %s:%d", flagship.BackEndIP.String(), srtPort)
	pullCommand := fmt.Sprintf("ffmpeg -y -hide_banner -loglevel info -i 'srt://%s:%d?streamid=read:egress/flagship&passphrase=%s&latency=8000&mode=caller&smoother=live&transtype=live' -c copy -f rtsp 'rtsp://127.0.0.1:%d/%s'", flagship.BackEndIP.String(), flagship.SrtIngestPort, flagship.Passphrase, rtspPort, flagship.Passphrase)
...
func (m *MediaMtxConfig) BuildFlagshipPath(srtPassphrase string, srtPort, rtspPort int) Paths {
	...
	flagshipDefault.RunOnReady = fmt.Sprintf("ffmpeg -y -hide_banner -loglevel info -i 'srt://127.0.0.1:%d?streamid=read:egress/flagship&passphrase=%s&mode=caller&smoother=live&transtype=live' -c copy -f rtsp 'rtsp://127.0.0.1:%d/%s'", srtPort, srtPassphrase, rtspPort, srtPassphrase)
...
func (m *MediaMtxConfig) BuildConfig(srtPassphrase string, pathsConfig Paths, srtPort int) MediaMtxConfig {
	...
	return MediaMtxConfig{
		...
		ApiAddress:        ":9997",
		ApiEncryption:     false,
		ApiServerKey:      "src.key",
		ApiServerCert:     "src.crt",
		ApiAllowOrigin:    "*",
		...
		PlaybackServerKey:      "src.key",
		PlaybackServerCert:     "src.crt",
		PlaybackAllowOrigin:    "*",
		...
		RtmpEncryption: "no",
		RtmpsAddress:   ":1936",
		RtmpServerKey:  "src.key",
		RtmpServerCert: "src.crt",
