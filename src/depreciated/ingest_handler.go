...
// InitFlagshipIngest initializes mediaMTX with a config that allows a user to push an SRT stream to the src
func InitFlagshipIngest(srtPort, rtspPort int) error {
	var config types.MediaMtxConfig
	var paths types.Paths
	paths = config.BuildFlagshipPath(Haven.Flagship.Passphrase, srtPort, rtspPort)
	config = config.BuildConfig(Haven.Flagship.Passphrase, paths, srtPort)
...
func InitEscortIngest(srtPort, rtspPort int) error {
	var config types.MediaMtxConfig
	var paths types.Paths
	paths = config.BuildEscortPath(Haven.Flagship, srtPort, rtspPort)
	config = config.BuildConfig(Haven.Flagship.Passphrase, paths, srtPort)
