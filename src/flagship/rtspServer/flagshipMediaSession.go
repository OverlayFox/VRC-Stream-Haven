...
import (
	...
	"github.com/OverlayFox/VRC-Stream-Haven/flagship/apiServer"
...
func (fh *FlagshipHandler) OnDescribe(ctx *gortsplib.ServerHandlerOnDescribeCtx) (*base.Response, *gortsplib.ServerStream, error) {
	...
	go func() {
		...
		closestEscorts := depreciated.Haven.GetClosestEscort(city)
		if closestEscorts[0].IpAddress.Equal(depreciated.Haven.Flagship.IpAddress) {
		...
		for _, escort := range closestEscorts {
			if escort.IpAddress.Equal(depreciated.Haven.Flagship.IpAddress) {
				continue
			}

			if !apiServer.IsApiOnline(escort) {
				logger.HavenLogger.Warn().Msgf("Escort %s is not reachable. Removing from Haven", escort.BackEndIP.String())
				err := depreciated.Haven.RemoveEscort(escort.IpAddress)
				if err != nil {
			...
			readers, err := apiServer.GetEscortReaders(escort)
			if err != nil {
