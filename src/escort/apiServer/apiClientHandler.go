...
import (
	...
	"github.com/OverlayFox/VRC-Stream-Haven/flagship/apiServer/register"
...
func RegisterEscortWithHaven(escort *types.Escort, flagshipIp net.IP, flagshipApiPort int, passphrase []byte) error {
	...
	encryptedBody, err := crypto.Encrypt(string(jsonData))
	...
	bodyJson, err := crypto.Decrypt(string(responseBody))
	if err != nil {
		return err
	}
	...
	depreciated.MakeHaven(flagshipEscort, decodedBody.SrtPort, string(passphrase))
	logger.HavenLogger.Info().Msgf("Successfully registered Escort with Flagship at %s", depreciated.Haven.Flagship.IpAddress.String())
