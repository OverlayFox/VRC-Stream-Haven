...
func NodeHlsPlaylist(rtmpUri string) error {
	...
	dataFolder := filepath.Join(dir, "/streams/%v/data_%02d.ts")
	masterPlaylistFolder := filepath.Join(dir, "/streams/stream_%v.m3u8")
	...
	hlsSegments := []string{"-hls_segment_type", "mpegts", "-strftime_mkdir", "1", "-hls_segment_filename", dataFolder, "-master_pl_name", "playlist.m3u8", "-master_pl_publish_rate", "3"}
	streamMap := []string{"-var_stream_map", "v:0,a:0,name:720p", masterPlaylistFolder}
