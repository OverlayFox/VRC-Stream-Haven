...
func NodeHlsPlaylist(rtspUri string) error {
	...
	hlsSegments := []string{"-hls_segment_type", "mpegts", "-strftime_mkdir", "1", "-hls_segment_filename", "C:/temp/hls-streams/%v/data_%02d.ts", "-master_pl_name", "playlist.m3u8", "-master_pl_publish_rate", "3"}
	streamMap := []string{"-var_stream_map", "v:0,a:0,name:720p", "C:/temp/hls-streams/stream_%v.m3u8"}
